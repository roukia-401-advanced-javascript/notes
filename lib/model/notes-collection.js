...
// action : list, payload:school
    async list(options) { //get
        ...
        if (options.payload === 'Please add a note') {
            ...
            let notesSameCategory = await notesSchema.find({ category: options.payload });//because its stored in the note place when the user hits the command
            console.log("notesSameCategory : ", notesSameCategory); // array of objects
            return notesSameCategory;
        }
